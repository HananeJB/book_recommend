...
def get_similar_books_all_read():
    id=int(request.args.get('id')) #id of user
    param = int(request.args.get('param')) # variable of how many similar books to pick by each book in the list
    num_books =int(request.args.get('num_books')) #how many books do you want to show to the user in total
    all_books = []
    new_user = df_user[df_user['id'] == id].iloc[0].to_frame().T
    #if user never read a book
...
def get_by_popularity():
    ...
    df_latest = df_latest.loc[(df['created_at'].dt.month == today.month)]
    df_latest = df_latest.sort_values('rating', ascending=False)
    print(df_latest['id'])
    result=json.loads(df_latest['id'].head(num).to_json(orient='table',index=False))

    return result

@app.route('/get_similar_books_by_recent', methods=['GET'])

def get_similar_books_by_num_of_recent():
    id=int(request.args.get('id')) #id of user
    param = int(request.args.get('param'))
    ...
    if pd.isnull(new_user.books.values[0]) == True:
        ...
        if len(new_user.books.values[0])!=1:

            books = new_user.books.values[0].split(',')
            books = [int(item) for item in books]

            for id in books[0:param]:
                books = df[df['id'] == id].iloc[0].to_frame().T
